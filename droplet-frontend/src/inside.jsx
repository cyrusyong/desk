/* global React, PixelMap */
const { useState, useMemo } = React;

/* =========================================================
   Envelope sprite (pixel art) — used in inside-the-box view
   ========================================================= */
function buildEnvelope(opened = false, withLetter = false) {
  const W = 56;
  const Ht = 36;
  ...
  // back flap (shown when opened): a triangle going up from top edge
  if (opened) {
    // open flap behind body — pointing up & away
    for (let i = 0; i < 12; i++) {
      const lx = W / 2 - i;
      const rx = W / 2 + i;
      const y = 1 + i;
    ...
    // flap interior fill (slightly darker paper)
    for (let i = 0; i < 11; i++) {
...
/* =========================================================
   Inside-the-box view: a pixel-tiled "interior" room with
   envelopes laid out in a grid. Hover wiggles. Click opens letter.
   ========================================================= */
...
function InsideView({ files, visible, onBack, onOpenLetter, px = 3 }) {
  // Visible when state === 'inside' or 'letter'
  return (
    <div
      style={{
        position: "fixed",
    ...
    >
      {/* Pixel "wall" texture — repeating cool-blue brick */}
      <div
        style={{
          position: "absolute",
          inset: 0,
...
/* =========================================================
   Letter overlay: full-screen card slides up from bottom,
   envelope flap opens, letter slides out, file details visible.
   ========================================================= */

function LetterOverlay({ file, onClose }) {
  if (!file) return null;
  const url =
    file.url ||
    `https://droplet-files.s3.us-east-2.amazonaws.com/${encodeURIComponent(
      file.name,
    )}`;
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 18, color: "#7a4a32" }}>
                from: us-east-2 &middot; bucket droplet-files
              </div>
            </div>
            <div
              style={{
          ...
          <Row label="field id" value={file.fieldId || randomId()} mono />
...
function Row({ label, value, mono }) {
  return (
    <div style={{ display: "flex", gap: 12, marginBottom: 8, alignItems: "baseline" }}>
      ...
      <div
        style={{
          fontFamily: mono ? "'VT323', monospace" : "'VT323', monospace",
          fontSize: 20,
          color: "#1b2a3a",
          flex: 1,
          wordBreak: "break-all",
...
window.InsideView = InsideView;
window.LetterOverlay = LetterOverlay;
